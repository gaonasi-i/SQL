...
CREATE TABLE employee(
    ...
    --ON DELETE CASCADB(부서를 삭제하면 참조하고 있는 사원도 삭제)
);

--참조 무결성 제약 위배(외래키)
--1. 사원이 소속된 부서를 삭제할때 '자식 레코드가 있다'는 오류가 발생
--2. 부서번호가 없는 사원자료를 추가할때 '부모키가 없다'는 오류가 발생
...
INSERT INTO employee VALUES (103, '김하늘', 3500000, SYSDATE, '', '1003');
...
-- 성별이 남자이면서 급여가 250만원 이상인 사원을 검색하세요
SELECT * FROM employee
WHERE gender = '남자'
AND sal >= 2500000;

-- 급여가 200만원 이상, 300만원 미만인 사원을 검색하시오. (급여: 200~300만원)
-- BETWEEN ~ AND
SELECT * FROM employee
...
--GROUP BY 절(소속등 그룹화) ,조건별 - HAVING 사용
--부서별 급여 총액을 구하시오
