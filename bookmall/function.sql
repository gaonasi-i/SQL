...
--SUBSTR(문자, 인덱스, 글자수)
SELECT SUBSTR('ABC', 1, 2) REPLACE FROM DUAL;

--REPLACE(문자, 이전문자, 새로운문자)
SELECT REPLACE('ABC', 'A', 'b') REPLACE FROM DUAL;

--CONCAT(문자1, 문자2) - 문자연결
SELECT CONCAT('A', 'B') REPLACE FROM DUAL;

--연결연산자 -'||'
SELECT '안녕' || '하세요' REPLACE FROM DUAL;

--LPAD(문자, 문자수, 기호) - 기호를 왼쪽부터 채움
SELECT LPAD('cloud', 10, '*') REPLACE FROM DUAL;
--LPAD(문자, 문자수, 기호) - 기호를 오른쪽부터 채움
SELECT RPAD('cloud', 10, '*') REPLACE FROM DUAL;
