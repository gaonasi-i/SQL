...
--JOIN(공통조인-EQUI JOIN) - 공통된 부분 (교집합, AND)
--고객의 이름과 고객이 주문한 도서의 판매가격을 구하시오
SELECT cus.name, ord.saleprice 판매금액
...
--고객별로 주문한 모든 조서의 총 판매액을 구하시오(SUM()함수)
SELECT cus.name, SUM(ord.saleprice) 판매금액
FROM customer cus, orders ord
WHERE cus.custid = ord.custid
GROUP BY cus.name;

--'안산'고객의 주문한 모든 조서의 총 판매액을 구하시오(SUM()함수, HAVING 구문)
SELECT cus.name, SUM(ord.saleprice) 판매금액
FROM customer cus, orders ord
WHERE cus.custid = ord.custid
GROUP BY cus.name
...
SELECT cus.name, bk.bookname, ord.saleprice 판매금액
FROM customer cus, orders ord, book bk
WHERE cus.custid = ord.custid
    AND bk.bookid = ord.bookid;
    
--'박지성' 고객이 주문한 도서의 이름과 판매가격, 주문일을 구하시오
SELECT cus.name, bk.bookname, ord.saleprice 판매금액, ord.orddata
FROM customer cus, orders ord, book bk
WHERE cus.custid = ord.custid
    AND bk.bookid = ord.bookid
    AND cus.name = '박지성';
...
--WHERE 절에서 모든 행이 출력되는 테이블의 반대편 테이블에 '+'를 붙힌다
